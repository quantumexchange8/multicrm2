...
const { getChannelName, formatDate, formatAmount, getStatusClass, formatTime } = transactionFormat();
const accountToWallets = ref({data: []});
const search = ref('');
const date = ref('');
...
const getResults = async (page = 1, search = '', dateRange) => {
    ...
    try {
        ...
        accountToWallets.value = response.data.AccountToWallet;
    } catch (error) {
        console.error(error);
    } finally {
    ...
    <div class="px-6 py-4 mt-6 overflow-hidden bg-white rounded-md shadow-md dark:bg-dark-eval-1">
        <div class="relative overflow-x-auto sm:rounded-lg mt-4">
            ...
            <table v-else class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                <tr v-for="accountToWallet in accountToWallets.data" class="bg-white odd:dark:bg-transparent even:dark:bg-dark-eval-0 text-xs font-thin text-gray-900 dark:text-white text-center">
                    <th scope="row" class="px-2 py-4 font-thin rounded-l-full">
                        {{ formatDate(accountToWallet.created_at) }}
                    </th>
