...
const rebateToAccounts = ref({data: []});
const search = ref('');
const date = ref('');
...
const getResults = async (page = 1, search = '', dateRange) => {
    ...
    try {
        ...
        rebateToAccounts.value = response.data.RebateToWallet;
    } catch (error) {
        console.error(error);
    } finally {
    ...
    <div class="px-6 py-4 mt-6 overflow-hidden bg-white rounded-md shadow-md dark:bg-dark-eval-1">
        <div class="relative overflow-x-auto sm:rounded-lg mt-4">
            ...
            <table v-else class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                <tr v-for="rebateToAccount in rebateToAccounts.data" class="bg-white odd:dark:bg-transparent even:dark:bg-dark-eval-0 text-xs font-thin text-gray-900 dark:text-white text-center">
                    <th scope="row" class="px-2 py-4 font-thin rounded-l-full">
                        {{ formatDate(rebateToAccount.created_at) }}
                    </th>
                    <th class="px-2 py-4 font-thin">
                        $ {{ formatAmount(rebateToAccount.amount) }}
                    </th>
                    <th class="px-6 py-2 font-thin rounded-r-full">
                        <Badge :status="getStatusClass(rebateToAccount.status)">{{ $t('public.' + rebateToAccount.status) }}</Badge>
                    </th>
                </tr>
                </tbody>
            </table>
